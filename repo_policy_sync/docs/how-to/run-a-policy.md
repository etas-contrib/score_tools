...
On a normal apply run, an existing policy PR whose branch was generated by the
current tool revision and is not in conflict is left unchanged. The generated
PR body includes a hidden generation marker so a tool revision change causes
the branch to be rebuilt from the repository's current default branch. PRs
without that marker are migrated through the same guarded rebuild path. If
GitHub reports a merge conflict, the tool also recreates the branch from the
current default branch and reapplies the policy.

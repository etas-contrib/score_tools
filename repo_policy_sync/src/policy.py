...
def load_policies(paths: tuple[Path, ...] | None = None) -> tuple[Policy, ...]:
    ...
    for policy in policies:
        identifiers = (policy.id, *policy.legacy_names)
        if len(set(policy.legacy_names)) != len(policy.legacy_names):
            raise PolicyError(f"policy legacy names must be unique: {policy.id}")
        if policy.id in policy.legacy_names:
            raise PolicyError(
                f"policy {policy.id} cannot list its current ID as a legacy name"
            )
        for identifier in identifiers:
            previous_name_owner = names.get(identifier)
            if previous_name_owner is not None and previous_name_owner != policy.id:
                raise PolicyError(
                    f"policy name {identifier!r} is claimed by both "
                    f"{previous_name_owner!r} and {policy.id!r}"
                )
            names[identifier] = policy.id
            slug = policy_branch_slug(identifier)
            if not slug:
                raise PolicyError(
                    f"policy ID cannot produce a branch name: {identifier!r}"
                )
...
def resolve_policy_names(
    ...
    available_path_by_resolved_path: dict[Path, Path] = {}
    for policy_directory in _policy_directories(directory):
        for path in discover_policy_paths(policy_directory):
            available_path_by_resolved_path.setdefault(path.resolve(), path)
    available_paths = tuple(sorted(available_path_by_resolved_path.values(), key=str))
...
def load_policy(path: Path) -> Policy:
    ...
    legacy_names = _string_list(raw.get("legacy_names", []), "legacy_names", path)
    if policy_id in legacy_names:
        raise PolicyError(
            f"policy {path}: legacy_names cannot contain the current policy ID"
        )
    if len(set(legacy_names)) != len(legacy_names):
        raise PolicyError(f"policy {path}: legacy_names must be unique")
    (
        bazel_condition,

...
def test_module_policy_pull_request_includes_the_matching_rationale() -> None:
    ...
    assert "This policy applies to configuration in `MODULE.bazel`." in body
    assert (
        "  - Remove module metadata that should be derived by the Bazel Registry."
    ) in body


def test_devcontainer_standardization_pull_request_explains_applicability_conditions() -> (
    None
):
    policy = load_policy(
    ...
    assert (
        "This repository matches this policy because `MODULE.bazel` exists and "
        "`.devcontainer/Dockerfile` matches this policy's file-content condition."
    ) in body
    assert "legacy content" not in body
